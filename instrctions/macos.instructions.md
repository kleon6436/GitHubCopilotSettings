...
- **ナビゲーション要素へのカスタム背景適用を禁止する。**  
  ...
  **例外**: 没入型コンテンツアプリ（写真・地図・星空観察など）では `.toolbarBackground(.hidden, for: .windowToolbar)` を使用し、コンテンツをツールバー背後まで広げるディストラクションフリーなデザインが適切な場合がある。Apple HIG も「Consider temporarily hiding toolbars for a distraction-free experience」として明示的に許容している。
